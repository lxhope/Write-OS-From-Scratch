...
%macro WriteSector 2  ; 2个参数，1: 驱动器编号，2: Disk Access Packet

        pusha

        mov dl, %1
        mov si, %2
        call bios_lba_write_sector
...
[ORG 0x7c00]
    ...
    mov al, 'C'                                 ; 将第二个扇区的第一个字符改为‘C’，并写入磁盘
    mov [second_sector_msg], al
    WriteSector [drive_num], DiskAdressPacket ; 写入启动磁盘扇区

    ReadSector [drive_num], DiskAdressPacket   ; 再次读取第扇区
